...
export const updateForumRouteName = "forum/update/:id";
const UpdateForum = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (data?.data) {
      let temp: CreateForumForm = {
        date: data.data.date,
        image: { image_link: "", image_url: data.data.image },
    ...
    <ContentContainer>
      <form onSubmit={handleCreate}>
        ...
        <div className="grid grid-cols-2 gap-5 mt-4">
          <ImageInput
            isWide={true}
            imagePreview={imagePreview ? imagePreview : data?.data.image}
